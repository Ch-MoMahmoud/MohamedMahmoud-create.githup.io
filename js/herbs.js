(async function(){
  ...
  try{
    ...
    const herbs = (data.products||[]).filter(p=> p.categoryId === 'c-herbs');
    ...
    const renderPage = (page=1)=>{
      ...
      items.forEach((p, idx)=>{
        ...
        } else if(p.featured){ const badge = el('span','product-badge product-badge--new'); badge.textContent = 'جديد'; a.appendChild(badge);}    
