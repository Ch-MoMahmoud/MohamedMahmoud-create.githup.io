(async function(){
  ...
  // Helper to get fallback data from inline script
  const getInlineData = () => {
  ...
  let data = null;
  try{
    // If running from file:// most browsers block fetch to local files
  ...
  try{
    ...
    const honey = (data.products||[]).filter(p=> p.categoryId === 'c-honey');
    ...
    const renderPage = (page=1)=>{
      ...
      items.forEach((p, idx)=>{
        ...
        if(p.discount && p.discount.type === 'percentage' && typeof p.discount.value === 'number'){
          const sale = el('span','product-badge product-badge--sale');
          sale.textContent = `${p.discount.value}%`;
          a.appendChild(sale);
        } else if(p.featured){
          const badge = el('span','product-badge product-badge--new');
