(async function(){
  ...
  try{
    ...
    // sort by optional sortOrder
    products.sort((a,b)=> (Number(a.sortOrder ?? 9999)) - (Number(b.sortOrder ?? 9999)));
    ...
    const applyFilters = (items)=>{
      ...
      const byPrice = bySearch.filter(p=> (Number(p.price)||0) >= minV && (Number(p.price)||0) <= maxV);
    ...
    const renderCards = (items)=>{
      ...
      items.forEach((p, idx)=>{
        ...
        // Do not bind manually; allow cart.js autoBind/MutationObserver to bind once
